...
IMAGE_DIR_MERGE_STR = "pixel\images"
SCAN_NUMBER_MERGE_STR = "S%03d"
TIFF_FILE_MERGE_STR = "px%05d.img"
SCANLOG_MERGE_STR = "spec\scanlogs"
SCANLOG_FILE_MERGE_STR = "x04v_scan%05d_log.dat"

class X04SASpecDataSource(SpecXMLDrivenDataSource):
    '''
    ...
    def __init__(self, 
        ...
        '''
        super(X04SASpecDataSource, self).__init__(projectDir, 
                                                     projectName, 
                                                     projectExtension,
                                                     instConfigFile, 
                                                     detConfigFile, 
...
#        scan = self.sd[scanNo]
    ...
    def getUBLattice(self, scan):
        """
        Read lattice corresponding to UB matrix either from SPEC
        """
        # from the #G3 line from the spec file. 
        try:
            g1 = scan.G["G1"].strip().split()
            lattice = np.array(list(map(float, g1)))[:6]
            logger.debug("lattice " +str(lattice))
        except:
            logger.error("Unable to read lattice from G1")
            logger.error( '-'*60)
    ...
    def getUBMatrix(self, scan):
        """
        Read UB matrix either from SPEC or manual specified file
        """
        if self.UBmatrixFile == None:
        # from the #G3 line from the spec file. 
            try:
        ...
        # from manual specified file
            try:
                with open(os.path.abspath(self.UBmatrixFile),'r') as f:
                    xraw=f.read().split()
                    f.close()
                ...
                logger.error("Unable to read UB Matrix from " + str(self.UBmatrixFile))
                logger.error( '-'*60)
                traceback.print_exc(file=sys.stdout)
                logger.error('-'*60)
    ...
    def loadSource(self, mapHKL=False):
        ...
        # First determine base directory of project by going two levels up
        logger.debug("self.projectDir: " + self.projectDir)
        baseDir = os.path.abspath(os.path.dirname(os.path.dirname(self.projectDir)))
        logger.debug("baseDir: " + baseDir)
        self.specFile = os.path.join(self.projectDir, self.projectName + \
                                     self.projectExt)
        logger.debug(self.specFile)
        # Now generate ImageDirector
        imageDir = os.path.abspath(os.path.join(baseDir, IMAGE_DIR_MERGE_STR))
        logger.debug("imageDir: " + imageDir)
        self.imageFileTmp = os.path.join(imageDir, TIFF_FILE_MERGE_STR)
        logger.debug("self.imageFileTmp: " +self.imageFileTmp)
        
        # And ScanLog Directory to retrieve image names
        self.scanlogDir = os.path.abspath(os.path.join(baseDir, SCANLOG_MERGE_STR))
        logger.debug("self.scanlogDir: " + self.scanlogDir)
        ...
        # if needed load the flat field file
        ...
        try:
            ...
            for scan in self.scans:
                if (self.cancelLoad):
                    ...
                    # First get the correct image number from ScanLog
                    if True:
                        try:
                            ...
                            if self.ubMatrix[scan] is None:
                                raise X04SASpecFileException("UB matrix not found.")
                            ...
                            if self.mapHKL==True:
                                ...
                                try:
                                    self.ubLattice = self.getUBLattice(curScan)
                                    self.HtoQx[scan] = 2.*np.pi/self.ubLattice[0]
                                    self.KtoQy[scan] = 2.*np.pi/self.ubLattice[1]
                                    self.LtoQz[scan] = 2.*np.pi/self.ubLattice[2]
                                except:
                                    raise X04SASpecFileException("UB Lattice not found.")                            
                            ...
                            self.imageBounds[scan] = \
                                self.findImageQs(angles, \
                                                 ...
                                                 HtoQx=self.HtoQx[scan],
                                                 KtoQy=self.KtoQy[scan],
                                                 LtoQz=self.LtoQz[scan])
    ...
    def x04saPil100K_imgreader(self, imgfile):
        """
        Opens and reads 32-bit binary image files. Returns 2D numpy array.
        X04SA Pilatus 100K gives this output.
        """
        f=open(imgfile,'rb')
        xraw=f.read()
        f.close()
        ...
        for scannr in scans:
            ...
            if filterName != None:
                ...
                if filter_data is None:
                    raise IOError("Did not find filter source '" + \
                                  ...
                                  "instrument Config file")
            # get image numbers for this scan:
            scanlog = os.path.abspath(os.path.join(self.scanlogDir, SCANLOG_FILE_MERGE_STR % scannr))
            imgnrs = np.loadtxt(scanlog,skiprows=1,usecols=1,dtype=np.uint32)
            ...
            for ind in range(len(scan.data[list(scan.data.keys())[0]])):
                if imageToBeUsed[scannr][ind] and mask[ind]:    
                    # read tif image
                    logger.debug("Image: " + self.imageFileTmp % imgnrs[ind])
                    img = self.x04saPil100K_imgreader(self.imageFileTmp % imgnrs[ind])
...
#                                      str(scanAngle1[i][ind]))
        ...
        qx = qx*self.HtoQx[scans[0]]
        qy = qy*self.KtoQy[scans[0]]
        qz = qz*self.KtoQy[scans[0]]
...
class X04SASpecFileException(RSMap3DException):
    '''
    ...
    '''
    def __init__(self, message):
        super(X04SASpecFileException, self).__init__(message)
